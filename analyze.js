...
- fais très pédagogique, pas résumé.`;
  ...
  if (b.openaiKey) {
    try {
      ...
      const r=await fetch('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${b.openaiKey}`},body:JSON.stringify({model:'gpt-4o-mini',messages,temperature:.55,response_format:{type:'json_object'}})});
      const data=await r.json(); const txt=data.choices?.[0]?.message?.content; if(txt) return normalize(JSON.parse(txt),base);
    } catch(e) {}
  }
  if (b.geminiKey) {
    try {
      const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${b.geminiKey}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:.55,responseMimeType:'application/json'}})});
